...
df_with_outs <- generate_x_with_outs(df_ori$value, 0.025, generate_outs_unif) |>
  mutate(timestamp = df_ori$timestamp)


# 2. Plot data: origin vs pollution
# ---------------------------------
df_plot_ori_pollu <- bind_rows(df_ori, df_with_outs) |>
  rename(时间 = timestamp,
         `电力需求(MW)` = value,
         值类型 = is_anomaly) |>
  mutate(值类型 = ifelse(值类型 == 0, "正常", "异常"),
...
ggsave(
  paste0(asset_dir, "example-taylor-ori-pollu.png"),
  plot = p_ori_pollu,
  width = 8,
  height = 4
)
...
p_comps <- autoplot(comps_plot) +
  labs(x = "时间", y = "电力需求(MW)")
ggsave(
  paste0(asset_dir, "example-taylor-comps.png"),
  plot = p_comps,
  width = 8,
  height = 4
)
...
# ---------------------------------------------------------
df_plot_val_seas_sups_long <- df_with_outs |>
  mutate(季节调整后 = x_adj, 季节调整并平滑后 = mod$y) |>
  rename(时间 = timestamp, 原始 = value) |>
  select(-is_anomaly) |>
  pivot_longer(
...
p_val_seas_sups <- ggplot(df_plot_val_seas_sups_long,
...
ggsave(
  paste0(asset_dir, "example-taylor-val-seas-sups.png"),
  plot = p_val_seas_sups,
  width = 8,
  height = 4
)
...
# ----------------------------------------------------
df_plot_residual_long <- df_with_outs |>
  mutate(剩余项 = rem, 重估后的剩余项 = resid) |>
...
p_rem_resid <- ggplot(df_plot_residual_long,
                      aes(x = 时间, y = `电力需求(MW)`, color = `剩余/重估后的剩余项`)) +
  geom_line() +
  labs(x = "时间", y = "电力需求(MW)") +
  scale_color_manual(values = c("剩余项" = "#C0C0C0", "重估后的剩余项" = "#555555"))
ggsave(
  paste0(asset_dir, "example-taylor-rem-resid.png"),
  plot = p_rem_resid,
  width = 8,
  height = 4
)
...
df_IO_AO <- full_join(df_IO, df_AO, by = join_by(ind == ind)) |>
  mutate(labels = auto_recognize_AO_IO(AO, IO)$labels) |>
  arrange(ind) |>
  select(ind, lambda1, lambda2, labels)
...
df_plot_outs_labeled <- df_with_outs |>
  mutate(
    is_anomaly = as.factor(is_anomaly),
    异常检测来源 = case_when(
      row_number() %in% intersect(outs_ind_TSA, outs_ind_IQR) ~ "Chang & IQR",
      row_number() %in% outs_ind_TSA ~ "Chang",
      row_number() %in% outs_ind_IQR ~ "IQR",
      TRUE ~ "-"
    )
  ) |>
  rename(时间 = timestamp,
...
p_TSA_IQR <- ggplot(df_plot_outs_labeled, aes(x = 时间, y = `电力需求(MW)`)) +
  ...
  labs(y = "电力需求(MW)")
ggsave(
  paste0(asset_dir, "example-taylor-TSA-IQR.png"),
  plot = p_TSA_IQR,
  width = 8,
  height = 4
)
