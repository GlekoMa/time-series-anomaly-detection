...
generate_synthetic_arma <- function(pollution_rate, n = 1000) {
  ...
  df_with_outs <- generate_x_with_outs(
    x = arma_sim,
...
# 1. ARMA
# --------
df_metrics_methods_multi_arma <- do.call(
  rbind, lapply(seq(0.01, 0.2, 0.01), test_data_synthetic_arma)
)
...
df_plot_arma$method <- factor(
  df_plot_arma$method,
  levels = c("分解+Chang+IQR", "分解+IQR", "分解+Chang", "Chang")
)
df_plot_arma <- rename(
  df_plot_arma,
  污染率 = pollution_rate,
...
ggsave(
  ...
  plot = p_box_all, width = 8, height = 2
)
